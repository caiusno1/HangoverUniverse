...
testenv.id = "testenv";
testenv.doors =  ["DoorSpawnRight"];
testenv.doors2pos={};
testenv.doors2pos["DoorSpawnRight"]={x:1600,y:400};
...
*/
        ...
        // testsection
        var self=this;
        var Leben=self.game.Leben;

        QUnit.test( "Test Player States", function( assert ) {
          assert.ok( Leben.getLeben() == 100, "HP full " );
          Leben.setLeben(50);
          assert.ok(50==Leben.getLeben(), "Set HP ok");
          Leben.heal(10);
          assert.ok( Leben.getLeben() == 60, "General Healing ok (static)" );
        });

        QUnit.test( "Test Trigger", function( assert ) {
          player.x=500;
          player.y=500;
          self.registerevent(testTheTrigger,400,400,200,200,"Test");
          var temp=self.interact;
          self.interact=true;
          self.askevent();
          self.interact=temp;
          assert.ok( self.game.test.triggered, "Trigger was forced successfully" );
        });
...
function testTheTrigger(self,sender)
{
  self.game.test.triggered=true;
}
